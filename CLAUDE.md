...
- **首頁重設計 + 作品頁**：首頁（`modules/home`）改用區塊元件——`HomeBannerCarousel`／`HomeCategoryChips`／`HomePopularActivities`（熱門）／`HomeAnimateTypeRow`（依作品逛：圈圈為作品圖片圓形頭像＋名稱＋活動檔數徽章，無圖退回名稱首字）／`HomeOngoingActivities`。新增路由 `/works`（作品列表 `WorkListPage`，圈圈同款）與 `/works/:animateTypeId`（作品活動 `WorkActivitiesPage`）。
- **活動／作品 API（activityStore + activityApi）**：`getActivities({animateTypeId,limit})`（`/activities`，活動帶 `animateTypeId/animateTypeName`、支援 `?animateTypeId` 篩選）、`getPopularActivities(limit)`（`/activities/popular`，人氣）、`getWorks(limit)`（`/works`，只回有活動的作品，回 `{id,name,imageUrl,activityCount}`；`fetchWorks` 正規化為 `{id,name,imageUrl,count}`）、`getActivity(id)`（`/activities/{id}`，單筆公開活動，供商品頁取目前活動）。store actions：`fetchActivities`／`fetchPopularActivities`／`fetchWorks`／`fetchActivitiesByWork`／`getOrFetchActivity`（快取優先，未命中才打 `getActivity`；商品頁不再重撈整包活動清單）。
