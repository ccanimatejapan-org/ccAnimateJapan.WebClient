...
- **首頁重設計 + 作品頁**：首頁（`modules/home`）改用區塊元件——`HomeBannerCarousel`／`HomeCategoryChips`／`HomePopularActivities`（熱門）／`HomeAnimateTypeRow`（依作品逛）／`HomeOngoingActivities`／`HomeGuideSection`。新增路由 `/works`（作品列表 `WorkListPage`）與 `/works/:animateTypeId`（作品活動 `WorkActivitiesPage`）。
- **活動／作品 API（activityStore + activityApi）**：`getActivities({animateTypeId,limit})`（`/activities`，活動帶 `animateTypeId/animateTypeName`、支援 `?animateTypeId` 篩選）、`getPopularActivities(limit)`（`/activities/popular`，人氣）、`getWorks(limit)`（`/works`，只回有活動的作品，回 `{id,name,activityCount}`）。store actions：`fetchActivities`／`fetchPopularActivities`／`fetchWorks`／`fetchActivitiesByWork`。
